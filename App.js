...
const App = () => {
  ...
  const viewSizeAnim = useRef(new Animated.Value(0.65 * vh)).current;
    ...
    ImagePicker.openCropper({
      ...
      .then((img) => {
        setCroppedImages(
          croppedImages
            .slice(0, imgIndex)
            .concat([img], croppedImages.slice(imgIndex + 1))
        );
      })
      .catch(() => {
        setCroppedImages(
          croppedImages
            .slice(0, imgIndex)
            .concat(croppedImages.slice(imgIndex + 1))
        );
      });
  };
  ...
  return (
    <>
      <StatusBar backgroundColor={theme.name === "dark" ? dark2 : accent} />
      ...
      <SafeAreaView style={[styles.container, theme.styles.content]}>
        <View style={[theme.styles.accent, styles.container]}>
          <Page theme={theme} page={showPage} setShowPage={setShowPage} />

          <View style={styles.menu_bar}>
            <SvgXml
              style={theme.styles.accent}
              xml={logo}
              width="21"
              height="25"
            />
            ...
            >
              <SvgXml
                xml={`<svg><path fill="#fff" d="M3 18h18v-2H3v2zm0-5h18v-2H3v2zm0-7v2h18V6H3z"></path></svg>`}
                width="25"
                height="25"
              />
            </Pressable>
          ...
          {!!croppedImages.length && !predictions.length && !loading && (
            ...
            >
              <Text style={[styles.idButtonText, theme.styles.button]}>
                Identifiser
              </Text>
            </Pressable>
          )}
          ...
          {!!croppedImages.length && !!predictions.length && (
            ...
            >
              <SvgXml
                ...
                xml={`<svg><path fill="currentColor" d="M9,3V4H4V6H5V19A2,2 0 0,0 7,21H17A2,2 0 0,0 19,19V6H20V4H15V3H9M7,6H17V19H7V6M9,8V17H11V8H9M13,8V17H15V8H13Z"></path></svg>`}
                width="25"
                height="25"
              />
            </Pressable>
          ...
          >
            ...
            {(!predictions.length || !croppedImages.length) && (
              <View>
                <HelpItem
                  icon={"flower-tulip"}
                  text={"Ta eller velg et bilde av en art i norsk natur"}
                  theme={theme}
                />
                <HelpItem
                  icon={"crop-free"}
                  text={"Zoom inn til arten på bildet"}
                  theme={theme}
                />
                <HelpItem
                  icon={"image-multiple-outline"}
                  text={"Flere bilder gir sikrere gjenkjenning"}
                  theme={theme}
                />
                <HelpItem
                  icon={"cloud-search-outline"}
                  text={"Trykk «identifiser» for å se hva det kan være!"}
                  theme={theme}
                />
              </View>
            )}
...
const styles = StyleSheet.create({
  ...
  scrollView: {
    borderTopLeftRadius: 0.07 * vw,
    borderTopRightRadius: 0.07 * vw,
    paddingTop: 0.03 * vh,
    paddingHorizontal: 0.055 * vw,
    flexGrow: 0,
  },
